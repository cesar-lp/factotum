...
usually described by a constraint equation rather than by `∇f = 0`. This
...
Since `∇f` is perpendicular to `f`'s level curves and `∇g` is
perpendicular to the constraint curve, tangency between the curves forces
`∇f` and `∇g` to be parallel — one is a scalar multiple of the other.
...
extremum must satisfy `∇f = lambda*∇g`, together with the constraint
itself.
...
`∇f = lambda*∇g` alone is satisfied by an entire curve of points, and only
...
condition generalizes: at a constrained extremum, `∇f` must lie in the
span of the constraint gradients, `∇f = lambda_1*∇g_1 + ... + lambda_k*∇g_k`,
...
rarely violated in textbook problems: it assumes `∇g != 0` at the
...
Finally, the system `∇f = lambda*∇g` with `g = c` only produces
**candidates** — points where a constrained extremum could occur, the
same way `∇f = 0` only produces unconstrained candidates. The method
