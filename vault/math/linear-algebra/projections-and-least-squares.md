...
squares. Suppose $Ax = b$ has no solution because $b$ doesn't lie in
...
exactly; it's the one whose $Ax$ is the projection of $b$ onto the column
space, since that's the closest any $Ax$ can get to $b$. Writing out the
orthogonality condition — the residual $b - Ax$ must be perpendicular to
*every column* of $A$, i.e. $A^\top(b - Ax) = 0$ — and rearranging gives the
**normal equations**:
...
> [!card] recall
> Starting from the requirement that the residual $b - Ax$ be orthogonal
> to every column of $A$, derive the normal equations.
> ---
> Orthogonality to every column of $A$ means $A^\top(b - Ax) = 0$.
> Distributing gives $A^\top b - A^\top A x = 0$, i.e. $A^\top A x = A^\top b$ — the
...
$Pb$ gives the projected point in the column space without solving for
$x$ first. > [!card] recall
...
**symmetric**, $P^\top = P$. Idempotence is the geometric fact in disguise:
...
> - [ ] because $A^\top A$ is always the identity matrix
> - [ ] because projection matrices are always diagonal
> - [ ] it isn't generally true; it only holds when $A$ is square ^card-afzf

For $A^\top A$ to be invertible — which the normal equations require — $A$'s
columns must be linearly independent. If they aren't, $A^\top A$ is singular:
...
projection $Ax$ still is. The projection of $b$ still exists and is still
