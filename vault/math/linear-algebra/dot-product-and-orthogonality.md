...
Algebraically, for $x, y$ in $\mathbb{R}^n$:
...
where $\theta$ is the angle between the two vectors. These are not two
...
> [!card] mcq
> A dot product $x \cdot y$ is computed as $\sum_{i=1}^n x_i y_i$ from a pair of
> coordinate vectors. Which statement correctly connects this to angle?
> - [x] It equals $\|x\|\|y\|\cos(\theta)$, so its sign tells you whether the angle between x and y is acute, obtuse, or exactly 90 degrees
> - [ ] It equals $\|x\|\|y\|\sin(\theta)$, so it measures the area between x and y
> - [ ] It is unrelated to angle; angle can only be recovered from the cross product
> - [ ] It equals $\|x\|+\|y\|\cos(\theta)$, combining length and angle additively ^card-opn9
...
and $\|y\|$ are always nonnegative, the sign of $x \cdot y$ is exactly the sign
of $\cos(\theta)$. A positive dot product means the vectors point broadly
in the same direction (angle under 90 degrees); negative means broadly
opposite; and $x \cdot y = 0$ means the vectors are ==orthogonal== — perpendicular, ^card-75dw
...
which is consistent with the Pythagorean picture: $x \cdot x = \sum_i x_i^2$, so
...
$$
\operatorname{proj}_y(x) = \left(\frac{x \cdot y}{y \cdot y}\right) y
$$

The scalar $\frac{x \cdot y}{y \cdot y}$ scales $y$ by exactly the right amount so that
...
Why is the projection of x onto y defined as $\left(\frac{x \cdot y}{y \cdot y}\right) y$ rather than just $(x \cdot y) y$? :: The extra division by $y \cdot y$ normalizes for the length of y — without it, scaling y by any factor would change the projection, even though "how much of x lies along y's direction" shouldn't depend on how long the vector chosen to represent that direction happens to be. Dividing by $y \cdot y$ makes the formula depend only on y's direction, not its magnitude. ^card-hzsw
...
It follows immediately from the geometric identity, since $|\cos(\theta)|$
...
What does the Cauchy-Schwarz inequality bound, and when does it hold with equality? :: It bounds the size of a dot product by the product of the two vectors' individual lengths — $|x \cdot y|$ can never exceed $\|x\|\|y\|$. Equality holds exactly when the two vectors are parallel (one is a scalar multiple of the other), which is the case where $|\cos(\theta)|$ reaches its maximum value of 1. ^card-qpvr
...
$\{q_1, \ldots, q_n\}$ is an orthonormal basis and $x = c_1 q_1 + \cdots + c_n q_n$,
...
> [!card] mcq
> $\{q_1, \ldots, q_n\}$ is an orthonormal basis and x = c_1*q_1 + ... + c_n*q_n.
