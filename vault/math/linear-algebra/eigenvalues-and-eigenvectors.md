...
Geometrically, $Av$ lands on the same line through the origin as $v$
...
> [!card] mcq
> $Av = \lambda v$ holds for a nonzero $v$. Which of the following is true?
...
Why does the definition of an eigenvector explicitly require $v \neq 0$? :: Because $v = 0$ satisfies $Av = \lambda v$ for every matrix $A$ and every scalar $\lambda$ — it's a trivial solution that carries no information about the map. Excluding it is what makes "eigenvalue" a meaningful, matrix-specific quantity rather than a tautology. ^card-ofyo
...
$Av = \lambda v$ has a nonzero solution $v$, which is the same as asking
...
$n \times n$ matrix); its roots are the eigenvalues.
...
> [!card] recall
> A matrix $A$ is diagonalizable as $A = PDP^{-1}$. Describe what $P$ and
...
Symmetric matrices ($A^\top = A$) are the well-behaved special case: they are
