...
`eigenvalues-and-eigenvectors.md` diagonalized $A$ as $PDP^{-1}$ — but only
...
Read right to left, each factor is a distinct geometric step: $V^\top$
...
> ---
> $A = USV^\top$. Reading right to left: $V^\top$ rotates/reflects the input
...
> - [ ] the eigendecomposition $A = PDP^{-1}$
...
==square roots== of the eigenvalues of $A^\top A$ (which is symmetric, so ^card-27r5
