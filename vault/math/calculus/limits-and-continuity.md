...
> [!card] mcq
> `f(x) = (x^2 - 1)/(x - 1)` is undefined at `x = 1`. What is `lim_{x->1} f(x)`?
...
right. Each one-sided limit — `lim_{x->a^-}` from below, `lim_{x->a^+}` from
above — is its own statement, and the two-sided limit `lim_{x->a} f(x)`
...
> [!card] recall
> State the epsilon-delta definition of `lim_{x->a} f(x) = L`.
> ---
