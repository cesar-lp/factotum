...
`(±5, 0)`, and neither one alone is "the" function the equation describes.
...
stand. Every term containing y then picks up a factor of `dy/dx` by the
chain rule, exactly as `d/dx[sin(u)] = cos(u) * du/dx` does when `u` is a
function of x:
...
That `2y * dy/dx` term — not just `2y` — is the entire technique. Forgetting
...
> - [ ] `3*y^2 = 1`
> - [ ] `3*y^2 * dy/dx = dy/dx`
> - [ ] `y^3 * dy/dx = 1` ^card-it6y

Notice the answer, `dy/dx = -x/y`, depends on *both* x and y, not on x
...
otherwise look like they must be assumed. Let `f(x) = e^x`, so `f^-1(y) =
ln(y)` and `f'(x) = e^x = y`:

```
d/dx[ln(x)] = 1/f'(f^-1(x)) = 1/x
```
...
Why is `d/dx[ln(x)] = 1/x` derivable from `d/dx[e^x] = e^x` alone, without evaluating any new limit? :: Because ln is the inverse of exp, so its derivative follows directly from the inverse-function formula `(f^-1)'(y) = 1/f'(x)`: since `f'(x) = e^x = y` when `f(x)=e^x`, substituting gives `(ln)'(y) = 1/y`, reusing the already-known derivative of exp rather than computing a new one. ^card-q2nn

> [!card] mcq
> A student computes `d/dx[arcsin(x)]` by starting from `f(x) = sin(x)` and its inverse-function relationship. Which substitution correctly turns `f'(x) = cos(x)` into a function of x (the arcsin's argument) alone?
