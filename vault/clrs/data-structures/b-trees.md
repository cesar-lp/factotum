...
about page layout, buffer pools, or crash recovery — for those on-disk
storage-engine concerns, see `database-internals/btree-basics.md` and
`btree-implementation.md`, which this note does not repeat.
...
exactly like the height-balance already described at a more general
level in `btree-basics.md`.
...
is exactly the assumption `btree-basics.md` makes when it says a high
fanout keeps disk seeks low. A larger t means fewer, fatter nodes, so a
...
operation `btree-basics.md` describes at the conceptual level, just
performed pre-emptively here rather than reactively.

What is the effect on tree height when a node split cascades all the way up to the root? :: The root itself splits into two nodes, and a brand-new root is created above them holding the single promoted median key — the only way a B-tree's height increases, exactly as `btree-basics.md` already states for B-trees in general. ^card-rb9k
...
it is the same borrow-or-merge choice `btree-implementation.md` describes
as rebalancing; CLRS just specifies the exact threshold (t keys) that
triggers it.
