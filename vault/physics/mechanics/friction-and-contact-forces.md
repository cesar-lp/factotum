...
```
0 <= f_s <= mu_s*N        (static, not yet slipping)
f_k ≈ mu_k*N              (kinetic, once sliding)
```
...
the way `F_net = m*a` is — both are empirical approximations that break
...
On an incline at angle `theta`, gravity resolves into a component along
the surface and a component into the surface:

```
along the incline (down-slope):  m*g*sin(theta)
into the incline:                m*g*cos(theta)
...
string: since `F_net = m*a` with `m = 0` forces the net force on any
