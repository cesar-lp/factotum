...
What does a NAT router rewrite in outgoing packets, and how does it route replies back? :: A NAT-enabled router rewrites the source IP and port of outgoing packets from a private address to its own public address and a chosen port, keeping a translation table so replies can be routed back to the correct internal host. ^card-s1qp
